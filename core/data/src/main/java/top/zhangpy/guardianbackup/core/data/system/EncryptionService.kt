 ...
 */
class EncryptionService(private val fileRepository: FileRepository) {
    ...
    fun decryptFile(inputFileUri: Uri, outputFileUri: Uri, password: CharArray) {
        fileRepository.getInputStream(inputFileUri).use { fis ->
            ...
            val secretKey = SecretKeySpec(keyFactory.generateSecret(keySpec).encoded, "AES")
            val cipher = Cipher.getInstance("AES/GCM/NoPadding")
            cipher.init(Cipher.DECRYPT_MODE, secretKey, javax.crypto.spec.GCMParameterSpec(GCM_tag_LENGTH, iv))
